...
@onready var time_label = get_node_or_null("UI/TimerLabel")

var current_wave = 1
var zombies_to_spawn = 15
var zombies_spawned = 0
var ammo_spawn_timer = 5.0
var survival_time = 300.0
var game_won = false
...
func _ready():
	if timer:
		...
		print("ОШИБКА: Timer не найден в сцене survival.tscn!")
	start_wave()
...
func update_time_label():
	if time_label:
		var secs = max(0, int(survival_time))
		time_label.text = "Время: " + str(secs / 60) + ":" + str(secs % 60).pad_zeros(2)
...
func spawn_bosses_with_delay():
	for i in range(BOSSES_PER_WAVE):
		if spawn_points and spawn_points.get_child_count() > 0:
			...
			boss.health = 15 + (current_wave - 1) * 10   # ЕДИНЫЙ БАЛАНС
...
func _on_timer_timeout():
	...
	if zombies_spawned < zombies_to_spawn:
		...
		zombie.health = 3 + current_wave   # ЕДИНЫЙ БАЛАНС
		add_child(zombie)
		zombies_spawned += 1
	else:
		if timer: timer.stop()
...
func add_kill(type: String):
	...
	if player and player.has_method("add_xp"):
		player.add_xp(15)   # чуть больше опыта, чтобы быстрее качаться
	match type:
		"zombie":
			zombie_kills += 1
		"boss":
